...
GAME_CASES: Dict[int, dict] = {
    814759080:  {"nom": "Зитракс", "gen": "Зитракса", "dat": "Зитраксу", "ins": "Зитраксом"},
    485898893:  {"nom": "Мельник", "gen": "Мельника", "dat": "Мельнику", "ins": "Мельником"},
    1214336850: {"nom": "Череп", "gen": "Черепа", "dat": "Черепу", "ins": "Черепом"},
    460174637:  {"nom": "Влад", "gen": "Влада", "dat": "Владу", "ins": "Владом"},
    1313515064: {"nom": "Булгак", "gen": "Булгака", "dat": "Булгаку", "ins": "Булгаком"},
    1035739386: {"nom": "Крюк", "gen": "Крюка", "dat": "Крюку", "ins": "Крюком"},
    407221863:  {"nom": "Некит", "gen": "Некита", "dat": "Некиту", "ins": "Некитом"},
    1878550901: {"nom": "Егориус", "gen": "Егориуса", "dat": "Егориусу", "ins": "Егориусом"},
    924097351:  {"nom": "Краб", "gen": "Краба", "dat": "Крабу", "ins": "Крабом"},
}
...
def game_name(user) -> str:
    cases = GAME_CASES.get(user.id)
    if cases:
        return cases["nom"]
    return USER_MAPPING.get(user.id, user.full_name)


def pf(player: dict, prefix: str = "n") -> dict:
    return {
        prefix: player["name"],
        f"{prefix}_r": player["name_r"],
        f"{prefix}_d": player["name_d"],
        f"{prefix}_t": player["name_t"],
    }
...
GROW_POS = [
    ...
    "У {n_r} +{d} см! Теперь {s} см 📈",
    "{n} отрастил {d} см! Теперь {s} см 📈",
    "{n} подрос на {d} см! Теперь {s} см 📈",
    "+{d} см для {n_r}! Теперь {s} см 📈",
    "{n} нарастил {d} см! Теперь {s} см 📈",
    "{n} окреп на {d} см! Теперь {s} см 📈",
    "У {n_r} прирост {d} см! Теперь {s} см 📈",
    "{n} набрал {d} см! Теперь {s} см 📈",
    "У {n_r} встал на {d} см крепче! Теперь {s} см 🍆",
    "{n} нехило так налился! +{d} → {s} см 😏",
    "Член {n_r} оценил и вырос на {d} см! {s} см 🍆",
    "{n} потёр и выросло на {d} см! {s} см 😏",
    "{n} растёт как на дрожжах! +{d} → {s} см 🚀",
    "Утренний стояк дал {n_d} +{d} см! {s} см 🌅",
    "{n} сходил в качалку — подросло на {d} см! {s} см 💪",
    "Природа наградила {n_r} ещё на {d} см! {s} см 🌿",
    "{n} помолился и получил +{d} см! {s} см 🙏",
    "{n} проснулся больше на {d} см! {s} см 😳",
    "У {n_r} рост не остановить! +{d} → {s} см 🔥",
    "Бабушка {n_r} накормила — +{d} см! {s} см 🍲",
    "{n} медитировал и отрастил {d} см! {s} см 🧘",
    "Генетика {n_r} не подвела! +{d} → {s} см 🧬",
]
GROW_NEG = [
    ...
    "-{d} см у {n_r}. Теперь {s} см 📉",
    "{n} уменьшился на {d} см. Теперь {s} см 📉",
    "{n} просел на {d} см. Теперь {s} см 📉",
    "У {n_r} убыло {d} см. Теперь {s} см 📉",
    "{n} растерял {d} см. Теперь {s} см 📉",
    "У {n_r} не стоит... -{d} см. Теперь {s} см 😔",
    "{n} передёрнул и потерял {d} см. {s} см 💀",
    "Холодно было, -{d} см у {n_r}. {s} см 🥶",
    "{n} обмяк на {d} см. Теперь {s} см 😩",
    "{n} вышел из душа — минус {d} см. {s} см 🚿",
    "Стресс добрался до {n_r}! -{d} → {s} см 😰",
    "{n} засиделся за компом, -{d} см. {s} см 🖥️",
    "{n} перенервничал и потерял {d} см. {s} см 😬",
    "Кто-то наступил {n_d} на самооценку, -{d} см. {s} см 👞",
    "{n} сходил к врачу — плохие новости. -{d} → {s} см 🏥",
    "{n} залип в редит — атрофия {d} см. {s} см 📱",
    "Бывшая написала {n_d}... -{d} см. {s} см 💔",
    "У {n_r} ложная тревога. -{d} → {s} см 😞",
    "Понедельник взял своё у {n_r}: -{d} см. {s} см 😵",
]
GROW_ZERO = [
    ...
    "У {n_r} полустояк. Без изменений. {s} см 😐",
    "{n} попробовал, но без результата. {s} см 🫤",
    "Звёзды не сошлись для {n_r}. {s} см ♒",
    "{n} тужился, но тщетно. {s} см 😤",
    "У {n_r} стабильность! {s} см 🧘",
    "{n} сегодня на нуле. {s} см 😶",
    "Природа взяла выходной у {n_r}. {s} см 🌧️",
]
GROW_LUBE = [
    "Смазка спасла {n_r} от потерь! {s} см 🧴",
    "{n} намазался и не усох! {s} см 🧴",
    "Lube сработал — {n} не потерял ничего! {s} см 🧴",
    "{n} проскользнул мимо потерь! {s} см 🧴",
    "Смазка защитила {n_r}! Всё ещё {s} см 🧴",
    "{n} смазал и проскочил! {s} см 🧴",
    "Скользкий {n} увернулся от минуса! {s} см 🧴",
    "Смазка {n_r} не подвела — ноль потерь! {s} см ✨",
    "{n} был готов — смазка на месте! {s} см 🧴",
    "Lube впитался, а потери нет! {n} — {s} см 🧴",
]
FIGHT_CHALLENGE = [
    "{a} вызвал {d_r} на бой!",
    "{a} бросил вызов {d_d}!",
    ...
    "{a} наехал на {d_r}!",
    "{a} полез на {d_r}!",
    "{a} кинул перчатку {d_d}!",
    "Внимание! {a} против {d_r}!",
    "{a} решил помериться с {d_t}!",
    "{a} достал и положил рядом с {d_t}!",
    "{a} хочет сравнить с {d_t}! 😏",
    "{a} решил потягаться с {d_t}!",
    "{a} вытащил на {d_r}!",
    "{a} выкатил пушку на {d_r}!",
    "{a} зашёл с козырей на {d_r}!",
    "{a} подкрался к {d_d} сзади!",
    "Схватка: {a} vs {d}!",
    "{a} не выдержал и полез к {d_d}!",
    "{a} смотрит на {d_r} с вызовом!",
    "Замер линейками: {a} и {d}!",
    "{a} бросил {d_d}: ну чё, слабо?",
    "{a} решил проверить {d_r} на прочность!",
    "{a} пришёл за размером {d_r}!",
]
FIGHT_WIN = [
    ...
    "У {w_r} больше! +{t} см 💥",
    "{w} разнёс! +{t} см 💣",
    "{w} уничтожил оппонента! +{t} см 😤",
    "Безоговорочная победа {w_r}! +{t} см 🏆",
    "{w} снял скальп! +{t} см 🗡️",
    "{w} прошёлся катком! +{t} см 🚜",
    "Нокаут от {w_r}! +{t} см 🥊",
    "{w} забрал своё и чужое! +{t} см 😈",
    "{w} не оставил шансов! +{t} см ⚡",
    "{w} сделал соперника! +{t} см 🎯",
    "Чистая победа {w_r}! +{t} см ✨",
]
FIGHT_CONDOM = [
    "{l} надел кондом — потери 0! 🛡️",
    "Кондом спас {l_r} от потерь! 🛡️",
    "{l} защитился кондомом! 🛡️",
    "Кондом {l_r} принял удар на себя! 🛡️",
    "{l} остался цел благодаря кондому! 🛡️",
    "{l} предохранился — ничего не потерял! 🛡️",
    "Резинка {l_r} выдержала! Потери 0 🛡️",
    "{l} был в броне — кондом сработал! 🛡️",
    "Защита {l_r} не пробита! 🛡️",
    "{l} подстелил соломку — 0 урона! 🛡️",
]
FIGHT_VIAGRA = [
    "Виагра {w_r} удвоила добычу! 💊",
    "{w} на виагре — забрал x2! 💊",
    "Виагра сработала! Двойной улов для {w_r} 💊",
    "{w} под виагрой — двойная мощь! 💊",
    "Эффект виагры: {w} берёт вдвойне! 💊",
    "{w} стоит крепко — виагра x2! 💊",
    "{w} выпил таблетку — двойной хапок! 💊",
    "Синяя таблетка дала {w_d} x2! 💊",
    "{w} на допинге — удвоенный трофей! 💊",
    "Химия на стороне {w_r} — x2 забор! 💊",
]
GIFT_PHRASES = [
    "{g} подарил {r_d} {a} см 🎁",
    "{g} отдал {r_d} {a} см 🎁",
    "{g} кинул {r_d} {a} см 🎁",
    "{g} пожертвовал {r_d} {a} см 🎁",
    "{r} получил {a} см от {g_r} 🎁",
    "{g} отрезал {a} см и отдал {r_d} 🎁",
    "{g} поделился с {r_t} — {a} см 🎁",
    "{g} щедро отсыпал {r_d} {a} см 🎁",
    "{g} отпилил {a} см для {r_r} 🎁",
    "{g} скинул {r_d} {a} см по дружбе 🤝",
    "Акт доброты: {g} → {r}, {a} см 💝",
    "{r} в плюсе на {a} см от {g_r}! 🎁",
    "{g} отломил {r_d} кусочек — {a} см 🎁",
]
LOTTERY_WIN = [
    "{w} забирает всё — {s} см 🎰",
    "{w} срывает куш! Теперь {s} см 🎰",
    "Джекпот у {w_r}! Теперь {s} см 🎰",
    "Всё уходит {w_d} — {s} см 🎰",
    "{w} выиграл лотерею! Теперь {s} см 🎰",
    "Удача на стороне {w_r} — {s} см 🎰",
    "{w} урвал весь банк себе! {s} см 🎰",
    "Сегодня у {w_r} стоит удача! {s} см 🎰",
    "Фортуна выбрала {w_r}! {s} см 🍀",
    "{w} хапнул весь банк! {s} см 💰",
    "Рулетка крутанулась — {w} в плюсе! {s} см 🎰",
    "Барабан выбрал {w_r}! {s} см 🎲",
    "{w} забрал общак! Теперь {s} см 🤑",
    "Звёзды сошлись для {w_r}! {s} см ⭐",
]

PERSONAL_EVENTS = [
    ("{n} случайно открыл OnlyFans — вдохновился!", 3.0),
    ("К {n_d} во сне пришла Меган Фокс!", 2.5),
    ("{n} не дрочил 3 дня — накопительный эффект!", 4.0),
    ("Незнакомка в метро глянула на {n_r}... встал!", 1.5),
    ("{n} загуглил 'как увеличить' и нашёл рабочий способ!", 3.0),
    ("{n} съел шаурму с секретным соусом!", 2.5),
    ("Гадалка предсказала {n_d} большое будущее!", 3.5),
    ("У {n_r} сработал nofap!", 3.0),
    ...
    ("Бывшая {n_r} засмеялась в голосовом...", -3.0),
    ("{n} сравнил с другом в бане. Зря.", -2.5),
    ("Мама {n_r} зашла без стука...", -2.0),
    ("{n} увидел свой размер в зеркале зимой.", -1.5),
    ("Девушка {n_r} сказала 'размер не главное'...", -3.0),
    ("{n} залип в тикток на 5 часов — атрофия!", -2.5),
    ("У {n_r} перегрев от ноутбука на коленях!", -2.0),
    ("{n} посмотрел документалку про китов... депрессия.", -2.0),
    ("{n} узнал что средний 13.5 см... и расстроился.", -1.5),
    ("Стресс от сессии ударил по {n_d}!", -2.5),
    ("{n} попытался измерить линейкой — расстроился.", -1.5),
    ("{n} нашёл папин протеин — помогло!", 3.5),
    ("{n} выпил энергетик — прилив сил!", 2.0),
    ("{n} проспал 12 часов — организм отблагодарил!", 2.5),
    ("Друг {n_r} проиграл спор — передал энергию!", 2.0),
    ("{n} сделал 100 приседаний — тестостерон пошёл!", 3.0),
    ("{n} сходил в баню — расширение!", 2.5),
    ("К {n_d} пришло второе дыхание!", 3.0),
    ("{n} получил посылку с Али — что-то сработало!", 4.0),
    ("{n} съел устриц — прирост!", 2.5),
    ("{n} станцевал на тиктоке — гормон счастья!", 1.5),
    ("{n} надел тесные джинсы... ошибка.", -2.0),
    ("Одногруппница {n_r} назвала его 'братиком'...", -2.5),
    ("{n} уронил гантелю... не туда.", -3.0),
    ("{n} посидел на холодном — усушка!", -1.5),
    ("У {n_r} пропал вайфай — стресс!", -2.0),
    ("{n} прочитал комменты в интернете — депрессия.", -2.0),
    ("{n} попал под дождь — сжатие!", -1.5),
    ("Будильник {n_r} сработал в 6 утра — шок!", -2.0),
    ("{n} сломал зарядку — нервы ни к чёрту.", -1.5),
    ("{n} проиграл в доту — удар по эго.", -2.5),
]
...
GLOBAL_EVENTS = [
    ...
    ("Солнечное затмение! Магнитные бури:", -1.5),
    ("Кто-то скинул мотивационное видео!", 1.0),
    ("Вода в кране стала теплее — расширение!", 1.5),
    ("Общая медитация в чате:", 1.0),
    ("Декан отменил пары! Радость:", 2.0),
    ("Кондиционер сломался — жара:", 1.5),
    ("Wi-Fi пропал на час! Стресс:", -1.0),
    ("Полнолуние! Странные вещи происходят:", "shuffle"),
    ("Кто-то притащил торт — праздничный буст!", 1.5),
    ("В чат зашёл бывший одногруппник... неловкость:", -1.0),
]

EVENT_PERSONAL_CHANCE = 0.08
EVENT_GLOBAL_CHANCE = 0.05
...
def get_or_create_player(user_id: int, name: str) -> dict:
    if user_id not in game_data:
        cases = GAME_CASES.get(user_id, {})
        game_data[user_id] = {
            "name": cases.get("nom", name),
            "name_r": cases.get("gen", name),
            "name_d": cases.get("dat", name),
            "name_t": cases.get("ins", name),
...
async def cmd_game(message: Message):
    ...
    await reply_game(message,
        f"/dick — твой размер\n"
        f"/grow — вырастить (кд {fmt_cd(cooldowns['grow'])})\n"
        f"/fight — бой реплаем или именем (кд {fmt_cd(cooldowns['fight'])})\n"
        f"/fight Имя 5 — бой со ставкой\n"
        f"/gift 3 — подарить (реплай)\n"
...
async def cmd_grow(message: Message):
    ...
    s = p["size"]
    if lube_used:
        text = random.choice(GROW_LUBE).format(**pf(p), s=s)
    elif actual > 0:
        text = random.choice(GROW_POS).format(**pf(p), d=actual, s=s)
    elif actual < 0:
        text = random.choice(GROW_NEG).format(**pf(p), d=abs(actual), s=s)
    else:
        text = random.choice(GROW_ZERO).format(**pf(p), s=s)
    event = check_event(p)
    if event:
        text += f"\n{event}"
...
async def cmd_fight(message: Message, command: CommandObject):
    ...
    if message.reply_to_message and message.reply_to_message.from_user:
        ...
        if args:
            try:
                bet = round(float(args.replace(",", ".")), 1)
            except ValueError:
                pass
    elif args:
        ...
        bet_candidate = None
        try:
            bet_candidate = round(float(parts[-1].replace(",", ".")), 1)
            name_query = " ".join(parts[:-1])
        except ValueError:
            name_query = " ".join(parts)
        if name_query:
            for uid, p in game_data.items():
                if p["name"].lower() == name_query.lower() and uid != user.id:
                    dfn = p
                    if bet_candidate is not None:
                        bet = bet_candidate
                    break
        if dfn is None:
        ...
        await reply_game(message, "Реплай или имя соперника 👊")
        return
    ...
    lines.append(random.choice(FIGHT_WIN).format(**pf(winner, "w"), t=actual))
    if condom_used:
        lines.append(random.choice(FIGHT_CONDOM).format(**pf(loser, "l")))
    if viagra_used:
        lines.append(random.choice(FIGHT_VIAGRA).format(**pf(winner, "w")))
